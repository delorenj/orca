...
type MockTransport = {
  ...
  /** Only the appearance pass reads this; set it per-test when driving applyTerminalAppearance. */
...
describe('connectPanePty', () => {
  ...
  it('records a mode 2031 subscribe from the raw chunk boundary without answering', async () => {
    ...
    mockStoreState = {
      ...
      settings: {
        ...mockStoreState.settings,
        theme: 'light',
        // Real defaults ship this on; without it a light app theme resolves to the DARK
        // terminal palette, which is the mode a 2031 subscriber is actually seeded with.
  ...
  describe('a theme flip has exactly one CSI 997 owner (#9993)', () => {
    /** Connects a visible pane and returns everything the appearance pass needs. */
    async function connectSubscribedPane(): Promise<{
      transport: ReturnType<typeof createMockTransport>
      deps: ReturnType<typeof createDeps>
      pane: ReturnType<typeof createPane>
      appearanceManager: { getPanes: () => unknown[] }
      sends997: () => string[]
      dispose: () => void
    }> {
      const { connectPanePty } = await import('./pty-connection')
      ...
      // A TUI subscribes and stays subscribed — the only state in which a flip is pushed.
      captured.current?.('\x1b[?2031h')
      expect(deps.paneMode2031Ref.current.get(1)).toBe(true)
      expect(deps.paneLastThemeModeRef.current.get(1)).toBe('light')
      ...
      return {
        ...
        } as unknown as { getPanes: () => unknown[] },
        sends997: () =>
          (transport.sendInput as unknown as { mock: { calls: unknown[][] } }).mock.calls
            .flat()
            .map(String)
            .filter((data) => data.includes('\x1b[?997')),
        dispose: () => binding.dispose()
      }
    }

    /** Real xterm re-reports the color scheme synchronously from the options.theme setter
     *  (ThemeService.onChangeColors → CoreBrowserTerminal._reportColorScheme → onData). */
    function reportColorSchemeOnThemeWrite(pane: ReturnType<typeof createPane>): void {
      let stored: unknown
      Object.defineProperty(pane.terminal.options, 'theme', {
        ...
        set: (value: unknown) => {
          stored = value
          sendTerminalInputThroughPane(pane, '\x1b[?997;1n')
        }
      })
    }

    it('sends one report when xterm also re-reports from the theme write', async () => {
      const { applyTerminalAppearance } = await import('./terminal-appearance')
      const { getDefaultSettings } = await import('../../../../shared/constants')
      const connected = await connectSubscribedPane()
      reportColorSchemeOnThemeWrite(connected.pane)

      applyTerminalAppearance(
        connected.appearanceManager as never,
        { ...getDefaultSettings('/tmp'), theme: 'dark' },
        true,
        new Map(),
        connected.deps.paneTransportsRef.current,
        'false',
        connected.deps.paneMode2031Ref.current,
        connected.deps.paneLastThemeModeRef.current
      )

      // Two emitters fired; exactly one payload reached the PTY. Two would paint
      // `^[[?997;1n^[[?997;1n` in a shell that lost the subscription mid-flight (#9993).
      expect(connected.sends997()).toEqual(['\x1b[?997;1n'])
      expect(connected.deps.paneLastThemeModeRef.current.get(1)).toBe('dark')
      connected.dispose()
    })

    it('sends nothing more on an attribute-neutral re-apply', async () => {
      const { applyTerminalAppearance } = await import('./terminal-appearance')
      const { getDefaultSettings } = await import('../../../../shared/constants')
      const connected = await connectSubscribedPane()
      reportColorSchemeOnThemeWrite(connected.pane)
      const settings = { ...getDefaultSettings('/tmp'), theme: 'dark' as const }
      const apply = (override: Partial<typeof settings>): void => {
        applyTerminalAppearance(
          connected.appearanceManager as never,
          { ...settings, ...override },
          true,
      ...
      apply({})
      // Font/opacity tweaks re-run the whole appearance pass; neither owner may re-emit.
      apply({ terminalFontSize: settings.terminalFontSize + 2 })
      apply({ terminalFontSize: settings.terminalFontSize + 4 })

      expect(connected.sends997()).toEqual(['\x1b[?997;1n'])
      connected.dispose()
    })

    it('seeds a subscribe from the palette, so a dark palette under a light app theme is quiet', async () => {
      // #9993: the seed must use the pusher's own resolver. Under a light app theme with
      // terminalUseSeparateLightTheme off the pane paints on the DARK palette, so an app-mode seed
      // of 'light' would make the very next unrelated re-apply look like a flip and emit CSI 997.
      const { connectPanePty } = await import('./pty-connection')
      const { applyTerminalAppearance } = await import('./terminal-appearance')
      const { getDefaultSettings } = await import('../../../../shared/constants')
      const transport = createMockTransport('pty-id')
      ...
      expect(deps.paneLastThemeModeRef.current.get(1)).toBe('dark')
    ...
    it('still forwards a program-driven color-scheme answer outside the theme write', async () => {
      const connected = await connectSubscribedPane()

      // A TUI's own `CSI ?996n` is answered by xterm through the same onData stream. No theme
      // write is in flight, so the suppression window must not swallow it.
      sendTerminalInputThroughPane(connected.pane, '\x1b[?997;2n')

      expect(connected.sends997()).toEqual(['\x1b[?997;2n'])
      connected.dispose()
    })
  })
