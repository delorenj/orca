...
import { resolveTerminalPaneColorSchemeMode } from './terminal-appearance'
import { shouldDropXtermColorSchemePush } from './xterm-theme-write-color-scheme-push'
...
export function connectPanePty(
  ...
  const handleHiddenMode2031SubscribeFact = (): void => {
    ...
    const mode = resolveTerminalPaneColorSchemeMode(
  ...
  const forwardPtyInput = (data: string): void => {
    ...
    // Why: an app theme write makes xterm push its own CSI 997 at mode-2031
    // subscribers. maybePushMode2031Flip owns that push (it covers hidden/parked
    // panes and every pane the write touched), so drop the duplicate (#9993).
    if (shouldDropXtermColorSchemePush(data)) {
      return
    }
  ...
  const runDeferredConnect = (): void => {
    ...
    function observeLiveMode2031Chunk(data: string): void {
      ...
      const settings = useAppStore.getState().settings
      // Why seed the mode: maybePushMode2031Flip only pushes on a real change, so an
      // unseeded subscription would push on the next unrelated appearance re-apply.
      // Must use the pusher's own resolver, or a palette whose luminance differs from
      // the app mode seeds a value the first re-apply reads as a flip.
      deps.paneMode2031Ref.current.set(pane.id, true)
      deps.paneLastThemeModeRef.current.set(
        pane.id,
        resolveTerminalPaneColorSchemeMode(settings, getSystemPrefersDark())
      )
    }
